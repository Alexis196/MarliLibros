...
export default async function CheckoutPendingPage({ searchParams }: { searchParams: Promise<{ method?: string }> }) {
      ...
      <main style={{ background: '#F7F6F2', minHeight: '70vh' }}>
        <div className="max-w-xl mx-auto px-4 sm:px-6 py-16 sm:py-24 text-center">
          <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6" style={{ backgroundColor: '#C8A86B' }}>
            <span className="text-white text-3xl">{isCash ? '💵' : '⏳'}</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: '#345457', fontFamily: 'var(--font-playfair)' }}>
            {isCash ? '¡Pedido registrado!' : 'Tu pago está siendo procesado'}
          </h1>
          <p className="text-gray-500 text-sm mb-8">
            {isCash
              ? 'Te vamos a contactar por WhatsApp para coordinar la entrega. Si te queda más cómodo, te pasamos el alias del negocio para pagar por transferencia.'
