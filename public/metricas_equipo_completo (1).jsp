...
<body>
  ...
  <%
    ...
    int pendientesDir = Integer.parseInt(zCantEvaluadosRepDirectos) - Integer.parseInt(zCantObjConfDirectos);

    /* ---- Porcentaje INDIRECTO (equipo completo) ---- */
    /* FIX: se calcula siempre que haya evaluados totales, sin depender de zCantObjConfIndirectos */
  ...
  <style>
    ...
    @media (min-width: 375px) and (max-width: 767px) {
      ...
      .met-pendiente-txt { font-size: 11px !important; }
      .met-pendiente-txt strong { font-size: 12px !important; }
    }
      ...
      <div class="met-ring-wrap">
        <svg viewBox="0 0 160 160" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle class="met-ring-fill verde" id="ringInd" cx="80" cy="80" r="65"
                  transform="rotate(-90 80 80)"
                  stroke-dasharray="408.41" stroke-dashoffset="408.41"
                  data-pct="<%= porcentajeIndirectoStr %>"/>
        </svg>
        <div class="met-ring-label">
  ...
  <script>
  (function() {
    var CIRCUM = 2 * Math.PI * 65; /* r=65 → 408.41 */

    function animarAnillo(id) {
      var el = document.getElementById(id);
      if (!el) return;
      var pct    = parseFloat(el.getAttribute('data-pct')) || 0;
      var target = CIRCUM * (1 - pct / 100);
      /* El estado inicial (dashoffset = CIRCUM) ya está en el atributo SVG del HTML */
      /* Agregar la transición y animar al valor final con setTimeout para garantizar repaint */
      setTimeout(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      animarAnillo('ringDir');
      animarAnillo('ringInd');
    });
  })();
  </script>
