...
<body>
  ...
  <%
    ...
    String porcentajeIndirectoStr = String.format(java.util.Locale.US, "%.0f", porcentajeIndirecto);
    ...
    String circumStr  = String.format(java.util.Locale.US, "%.2f", CIRCUM);
    String offsetDirStr = String.format(java.util.Locale.US, "%.2f", offsetDir);
    String offsetIndStr = String.format(java.util.Locale.US, "%.2f", offsetInd);
  %>
  ...
  <style>
    ...
    .met-ring-fill { fill: none !important; stroke-width: 13 !important; stroke-linecap: round !important; }
      ...
      <div class="met-ring-wrap">
        <svg viewBox="0 0 160 160" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle class="met-ring-fill azul" id="ringDir"
                  ...
                  stroke-dashoffset="<%= offsetDirStr %>"/>
        </svg>
      ...
      <div class="met-ring-wrap">
        <svg viewBox="0 0 160 160" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle class="met-ring-fill verde" id="ringInd"
                  ...
                  transform="rotate(-90 80 80)"
                  stroke-dasharray="<%= circumStr %>"
                  stroke-dashoffset="<%= offsetIndStr %>"/>
        </svg>
        <div class="met-ring-label">
  ...
  (function() {
    function animarAnillo(id, offsetFinal) {
      ...
      if (!el) return;
      /* El elemento ya arranca con dashoffset = CIRCUM (cerrado, seteado en el HTML) */
      setTimeout(function() {
        el.style.transition = 'stroke-dashoffset 1s cubic-bezier(0.4, 0, 0.2, 1)';
        el.setAttribute('stroke-dashoffset', offsetFinal);
      }, 100);
    }

    function iniciarAnillos() {
      animarAnillo('ringDir', <%= offsetDirStr %>);
      animarAnillo('ringInd', <%= offsetIndStr %>);
    }

    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', iniciarAnillos);
    } else {
      iniciarAnillos();
    }
  })();
